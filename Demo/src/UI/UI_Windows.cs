...
using Voxand.Engine.Systems.General.Platform.FileSystemDialog;
...
public sealed class UI_MapSettings : UI_Window
{
    UI_Button saveMapButton;
    UI_Button loadMapButton;

    IWindowService windowService;

    const string mapFileExtension = "spkvox";

    public unsafe UI_MapSettings()
    {
        windowService = ServiceLocator.GetService<IWindowService>();
        ServiceLocator.AddReplacementCallback<IWindowService>((windowService) => this.windowService = windowService);

        saveMapButton = new("Save map", new NVec2(60, 20)) { SizeMode = SizeMode.FillWidth };
        loadMapButton = new("Load map", new NVec2(60, 20)) { SizeMode = SizeMode.FillWidth };
        ...
        saveMapButton.OnClick += () =>
        {
            SaveFileDialog dialog = new()
            {
                Title = "Save map",
                DefaultFileName = "map",
                DefaultExtension = mapFileExtension,
                WindowHandle = (void*)windowService.Win32Handle,
            };
            string? path = dialog.OpenDialog();
            if (path is not null)
            {
                string extension = Path.GetExtension(path);
                if (extension != $".{mapFileExtension}")
                {
                    Win32.MessageBox(0, $"Sparkvox map files use '.{mapFileExtension}' file extension, not '{extension}'. Save file was renamed accordingly.", "File extension altered", 0x00000000u);
                    path = Path.ChangeExtension(path, mapFileExtension);
                }

                using Stream exportStream = File.Create(path);
                ServiceLocator.GetService<IVoxelMapPersistence>().Export(exportStream);
            }
        };
        ...
        loadMapButton.OnClick += () =>
        {
            ...
            {
                Title = "Load map",
                Filters = $"Sparkvox Map (*.{mapFileExtension})\0*.{mapFileExtension}\0\0",
                WindowHandle = (void*)windowService.Win32Handle,
            };
            string? path = dialog.OpenDialog();
            ...
            if (path is not null)
            {
                using Stream importStream = File.Open(path, FileMode.Open);
                ServiceLocator.GetService<IVoxelMapPersistence>().Import(importStream);
            }
        };
    }
    protected override void Display()
    {
        ...
        saveMapButton.Display();
        loadMapButton.Display();
...
public sealed class UI_VoxelToolHotbar : UI_Window
{
    ...
    public UI_VoxelToolHotbar(VoxelTool tool, VoxelToolHotbar hotbar)
    {
        ...
        hotbarTable = new UI_Table(
            label: "hotbar",
            flags: ImGuiTableFlags.SizingStretchSame,
            columnInfos: [new UI_TableColumnInfo("techniques", ImGuiTableColumnFlags.WidthStretch, 100, 1, (row) => {
